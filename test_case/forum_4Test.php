...
class Forum_4Test extends CustomSelenium2TestCase
{
    ...
    {
        $this->url("/topic_list.php?forum_id=6&from=forumHomeBar");
        
        // .container
        $el = $this->byXPath('//*[@id="img_one"]');
        $this->assertTrue($this->_assertRequestStatus($el->attribute('src')), '圖片可能有破圖');
        $el = $this->byXPath('//*[@id="img_two"]');
        $this->assertTrue($this->_assertRequestStatus($el->attribute('src')), '圖片可能有破圖');
        $el = $this->byXPath('//*[@id="img_thr"]');
        $this->assertTrue($this->_assertRequestStatus($el->attribute('src')), '圖片可能有破圖');
